...
**優先用總會匯出檔。** Grafana 讀的 `gideons_goal1` 沒有新財年的目標資料
（2026-07 實測：FY2027 目標在總會系統是 弟兄會費 25、教會聖奉 100,000、
贈送聖經 4,000…，Grafana 卻回 null）。上傳官方匯出檔就能把
目標／差額／達成率一次補齊。

> **兩種版面欄位不同**：總會版整體左移兩欄（無空白 B/C 欄）。
> 硬編欄號會讀成完全錯誤的資料，所以 `parse_excel` 靠 Row 4 的
> 「日期／講員／名稱」三連欄自動辨識版面，兩種都有單元測試覆蓋。

### 為什麼不讓 App 自己登入總會系統

評估過但不建議：總會站用 NextAuth，要自動化就得把會員編號與生日
存在本機、複製登入流程，而那是未公開的內部 API，隨時可能改版。
「自行匯出 → 上傳」只多一個動作，卻不必保存個人憑證，也不會因對方
改版而壞掉。
